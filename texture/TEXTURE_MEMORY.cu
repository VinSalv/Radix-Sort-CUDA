...
/**
    @file TEXTURE_MEMORY.cu
*/

// PURPOSE OF THE FILE: Implementation of the RadixSort algorithm by using CUDA texture memory.
...
// Texture Reference
texture<unsigned int, 1> input_array_GPU_texture_reference;
texture<unsigned int, 1> cumulative_sum_array_per_block_texture_reference;
texture<unsigned int, 1> cumulative_sum_array_per_grid_texture_reference;
texture<unsigned int, 1> single_bit_texture_reference;
...
/*
* @brief Device function to get element from texture reference
* @param index    index of input_array_GPU_texture_reference.
*/
__device__ unsigned int get_input_array_GPU_texture_element(unsigned int index) {
    return tex1Dfetch(input_array_GPU_texture_reference, index);
}

/*
* @brief Device function to get element from texture reference
* @param index    index of cumulative_sum_array_per_block_texture_reference.
*/
__device__ unsigned int get_cumulative_sum_array_per_block_texture_element(unsigned int index) {
    return tex1Dfetch(cumulative_sum_array_per_block_texture_reference, index);
}

/*
* @brief Device function to get element from texture reference
* @param index    index of cumulative_sum_array_per_grid_texture_reference.
*/
__device__ unsigned int get_cumulative_sum_array_per_grid_texture_element(unsigned int index) {
    return tex1Dfetch(cumulative_sum_array_per_grid_texture_reference, index);
}

/*
* @brief Device function to get element from texture reference
* @param index    index of single_bit_texture_reference.
*/
__device__ unsigned int get_single_bit_texture_element(unsigned int index) {
    return tex1Dfetch(single_bit_texture_reference, index);
}
...
__global__ void counting_ones_per_grid(unsigned int *cumulative_sum_array_per_block, unsigned int *cumulative_sum_array_per_grid) {
    ...
    __shared__ int shared_cumulative_sum_array_per_grid[thread_per_block];

    shared_cumulative_sum_array_per_grid[current_thread] = get_cumulative_sum_array_per_block_texture_element(index_thread);

    for (i = 0; i < current_block; i++)
        shared_cumulative_sum_array_per_grid[current_thread] += get_cumulative_sum_array_per_block_texture_element((i * dim_block) + (dim_block - 1));
...
__global__ void sorting_per_bit(unsigned int *input_array_GPU, unsigned int *result_array, unsigned int *cumulative_sum_array_per_grid, unsigned int *single_bit) {
    ...
    if (get_single_bit_texture_element(index_thread)) 
        result_array[get_cumulative_sum_array_per_grid_texture_element(index_thread) - 1 + size_of_array - get_cumulative_sum_array_per_grid_texture_element(size_of_array - 1)] = get_input_array_GPU_texture_element(index_thread);
    else
        result_array[index_thread - get_cumulative_sum_array_per_grid_texture_element(index_thread)] = get_input_array_GPU_texture_element(index_thread);
}
...
void radix_sort_on_GPU(unsigned int *input_array, unsigned int *output_array_GPU, unsigned int *number_of_all_blocks, float *mflop_per_sec, float *elapsed_from_GPU) {
    ...
    cudaChannelFormatDesc channel_descriptor = cudaCreateChannelDesc<unsigned int>();
    ...
    *number_of_all_blocks = ((size_of_array / thread_per_block) * thread_per_block < size_of_array) ? ((size_of_array / thread_per_block) + 1) : (size_of_array / thread_per_block);
    ...
    check_error_from(cudaBindTexture(0, input_array_GPU_texture_reference, input_array_GPU, channel_descriptor));
    check_error_from(cudaBindTexture(0, cumulative_sum_array_per_block_texture_reference, cumulative_sum_array_per_block, channel_descriptor));
    check_error_from(cudaBindTexture(0, cumulative_sum_array_per_grid_texture_reference, cumulative_sum_array_per_grid, channel_descriptor));
    check_error_from(cudaBindTexture(0, single_bit_texture_reference, single_bit, channel_descriptor));
    ...
    *mflop_per_sec = perform_Mflop_per_sec(elapsed_in_ms, *number_of_all_blocks);
    ...
    check_error_from(cudaUnbindTexture(input_array_GPU_texture_reference));
    check_error_from(cudaUnbindTexture(cumulative_sum_array_per_block_texture_reference));
    check_error_from(cudaUnbindTexture(cumulative_sum_array_per_grid_texture_reference));
    check_error_from(cudaUnbindTexture(single_bit_texture_reference));
}
...
void make_csv_of(unsigned int number_of_all_blocks, float mflop_per_sec, float elapsed_from_GPU, double elapsed_from_CPU) {
    ...
    char filename[] = "../measures/texture_measures_";
